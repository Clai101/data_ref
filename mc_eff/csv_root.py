...
def process_file_MC(filename):
        try:
            with uproot.open(filename) as file:
                ...
                inc = tupl2.arrays(library='pd')
                inc['source_file'] = filename
                csv_path = f"{os.path.splitext(filename)[0]}_MC.csv"
                inc.to_csv(csv_path, index=False)  # Save as CSV
                print(f"Saved {csv_path}")
                return csv_path
        except Exception as e:
...
def main():
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_file_MC, filename) for filename in os.listdir() if filename.endswith('.root')]
        # Collect results as they are completed
        for future in as_completed(futures):
            csv_path = future.result()
            if csv_path:
