...
def process_file_DD(filename):
        try:
            with uproot.open(filename) as file:
                ...
                csv_path = f"{os.path.splitext(filename)[0]}_DD.csv"
...
def main():
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_file_DD, filename) for filename in os.listdir() if filename.endswith('.root')]
        # Collect results as they are completed
        for future in as_completed(futures):
            csv_path = future.result()
            if csv_path:
